...
class ScoreboardManager:
    ...
    def __init__(self, app_config: dict):
        ...
        logger.info(f"Refresh interval for in-progress favorite games: {self.refresh_intervals.get('in_progress_favorite_team')} seconds.")
        if self.weather_settings:
            logger.info(f"Weather display enabled for: {self.weather_settings.get('location')}")
        else:
    ...
    def _should_fetch_full_scoreboard(self, current_time: datetime) -> bool:
        ...
        return current_time - self.last_full_scoreboard_fetch_time >= self.REFRESH_INTERVAL_DAILY_FULL_SCOREBOARD
    ...
    def _should_fetch_game_by_id(self, game: Game, current_time: datetime) -> bool:
        ...
        if game.is_completed:
            ...
            is_favorite_game = any(team.id in self.FAVORITE_TEAM_IDS for team in [game.home_team, game.away_team])
            if is_favorite_game:
                return current_time - last_fetch >= self.REFRESH_INTERVAL_IN_PROGRESS_FAVORITE
            else:
    ...
    def run(self):
        ...
        logger.info("ScoreboardManager running (this is a placeholder for the main application logic).")
