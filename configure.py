...
def get_user_input(config: dict):
    ...
    if questionary.confirm("Do you want to enable Weather display?", default=config.get("weather", {}).get("enabled", False)).ask():
        weather_location = questionary.text(
            ...
            validate=lambda text: "Location cannot be empty." if not text.strip() else True
        ).ask()
        ...
        api_key = questionary.password(
            ...
            validate=lambda text: "API Key cannot be empty." if not text.strip() else True
        ).ask()

        # Validate the API Key
        if not validate_openweathermap_api_key(api_key, weather_location):
            print("Error: Invalid OpenWeatherMap API Key or location. Please try again.")
            ...
            config["weather"] = {
                ...
                "refresh_interval_sec": questionary.text(
                    ...
                    default=str(config.get("weather", {}).get("refresh_interval_sec", 600)),
...
def validate_openweathermap_api_key(api_key, location):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"API Key validation failed. Response: {data.get('message', 'Unknown error')}")
            return False
    ...
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
            try:
                ...
                print(f"Warning: Could not parse {CONFIG_FILE}. Starting with empty configuration.")
                current_config = {}
